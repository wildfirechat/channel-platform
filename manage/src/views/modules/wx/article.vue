            ...
            <el-table :data="dataList" border v-loading="dataListLoading" @selection-change="selectionChangeHandle" style="width: 100%;">
                ...
                <el-table-column fixed="right" header-align="center" align="center" width="150" label="操作">
                    <template slot-scope="scope">
                        <el-button type="text" size="small" @click="broadcastArticle(scope.row.id)">群发</el-button>
...
export default {
    ...
    methods: {
        ...
        // 群发
        broadcastArticle(id) {
            this.$confirm(`确定对 id=${id} 进行群发操作?`, '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
                this.$http({
                    url: this.$http.adornUrl(`/manage/article/send/${id}/${this.selectedAppid}`),
                    method: 'post'
                }).then(({ data }) => {
                    if (data && data.code === 200) {
                        this.$message({
